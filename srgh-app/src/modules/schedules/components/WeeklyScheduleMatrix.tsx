...
const SELECT_CLASSES =
  'w-full rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-800 shadow-sm transition focus:border-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-600/20 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500'
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="hidden md:block">
          <div className="overflow-x-auto">
            <table className="min-w-[1100px] w-full border-separate border-spacing-0 text-sm lg:min-w-[980px] xl:min-w-[1100px]">
              <thead>
                <tr className="bg-slate-50 text-[11px] uppercase tracking-[0.18em] text-slate-500">
                  <th className="sticky left-0 z-20 border-b border-slate-200 bg-slate-50 px-3 py-3 text-left font-bold lg:px-4">
                    Colaborador
                  </th>
                  {weekDates.map((dateISO, index) => (
                    <th
                      key={dateISO}
                      className="border-b border-slate-200 px-2 py-3 text-center font-bold lg:px-3"
                    >
                      <div className="space-y-1">
                        <div className="text-[10px] leading-tight lg:text-[11px]">
                          {DAY_NAMES[index]}
                        </div>
                  ...
                  <th className="border-b border-slate-200 px-3 py-3 text-center font-bold lg:px-4">
                    Total semanal
                  </th>
                </tr>
              </thead>
              <tbody>
                {rows.length === 0 ? (
                  ...
                  scheduleRows.map((row) => (
                    ...
                    >
                      <td className="sticky left-0 z-10 border-b border-slate-100 bg-white px-3 py-3 lg:px-4">
                        <div className="flex items-center gap-2 lg:gap-3">
                          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-indigo-600 to-sky-500 text-[11px] font-black text-white shadow-sm lg:h-10 lg:w-10 lg:text-xs">
                            {row.nombreCompleto
                              .split(' ')
                              .slice(0, 2)
                              .map((part) => part.charAt(0))
                      ...
                      {row.dias.map((assignment, index) => {
                        ...
                        return (
                          <td
                            key={assignment.fecha}
                            className="border-b border-slate-100 px-1.5 py-2 lg:px-2"
                          >
                            <div className="min-h-[84px] rounded-xl border border-slate-200 bg-slate-50/60 p-2 lg:p-2.5">
                              {canWrite ? (
                                <select
                                  className={SELECT_CLASSES}
                                  value={selectValue}
                              ...
                              <div className="mt-2 flex items-center justify-between text-[10px] font-semibold text-slate-500 lg:text-[11px]">
                                <span>
                                  {assignment.esDiaLibre
                                    ? 'Libre'
                                    : assignment.horaEntrada && assignment.horaSalida
                      ...
                      <td className="border-b border-slate-100 px-3 py-3 text-center lg:px-4">
                        <span className="inline-flex rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-black text-emerald-700 lg:px-3 lg:text-xs">
