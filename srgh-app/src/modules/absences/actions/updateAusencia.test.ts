...
import { updateAusencia } from './updateAusencia'
...
describe('updateAusencia (server action)', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('rechaza una fecha final anterior a la de inicio sin llamar a requirePermission', async () => {
    const result = await updateAusencia(9, { ...baseInput, fechaFin: '2026-01-01' })

    expect(result).toEqual({ ok: false, error: 'Datos de la ausencia invalidos.' })
    expect(mockRequirePermission).not.toHaveBeenCalled()
  })

  it('rechaza si se traslapa con otra ausencia distinta', async () => {
    ...
    const result = await updateAusencia(9, baseInput)
  ...
  it('recalcula dias naturales/habiles y actualiza en exito', async () => {
